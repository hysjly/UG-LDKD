...
class BaseRoIHeadv1(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
                 ...
                 output_mask_roi: bool = False) -> None:  # 新增参数
        super().__init__(init_cfg=init_cfg)
        self.train_cfg = train_cfg
        self.test_cfg = test_cfg
        self.output_mask_roi = output_mask_roi  # 存储参数
    ...
    def predict(self,
        ...
        if self.with_mask:
            results_list = self.predict_mask(
                ...
                results_list,
                rescale=rescale,
                # output_roi=self.output_mask_roi
            )  # 传递参数

        if self.output_mask_roi and self.with_mask:
            for results, proposals in zip(results_list, rpn_results_list):
                # 假设 proposals 中包含 RoI 信息
                # 将 RoI 信息添加到 InstanceData 中
                results.rois = proposals.bboxes  # 添加 RoI 区域信息

        return results_list

    @abstractmethod
    def predict_bbox(self, x, batch_img_metas, rpn_results_list, rcnn_test_cfg, rescale):
