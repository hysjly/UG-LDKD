...
class StandardRoIHead(BaseRoIHead):
    ...
    def init_get_mask_rois(self, x: Tuple[Tensor], rpn_results_list: InstanceList, batch_data_samples: SampleList = None) -> Tuple[Tensor, Tensor]:
        """获取与 mask_head 输出相关的 RoI 区域。

        Args:
            x (Tuple[Tensor]): 特征图列表，包含多尺度图像特征。
            rpn_results_list (InstanceList): 区域提议列表。
            batch_data_samples (SampleList): 批量数据样本（通常包含注释信息等）。

        Returns:
            Tuple[Tensor, Tensor]: 返回 mask_head 输出对应的 RoI 区域（`mask_rois`）和相关的特征（`mask_feats`）。
        """
        ...
        # 获取 mask_rois
        mask_rois = rois[:100]  # 假设你只关注前100个RoI（可以根据需要调整）
