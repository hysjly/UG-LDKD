...
from Datasetv2_1 import DataSet
from Model import Encoder
# torch.backends.cudnn.benchmark = False
data_path = '/home/jz207/workspace/liull/MMDetection/data/'  # 数据路径
json_path = '/data/Kvasir_SEG/coco_format.json'  # JSON 文件路径
set = 'Kvasir_SEG'  # 数据集类型
...
modelPath = '/home/jz207/workspace/liull/MMDetection/Dynamic_Kvasir/'
fracTrain = 80

device = DeviceInitialization('cuda:0')
trainIdx, testIdx = DataReading(data_path=path, set=set, fracTrain=fracTrain)
trainSet = DataSet(dataPath=path, set=set, dataName=trainIdx, height=256, width=256)
testSet = DataSet(dataPath=path, set=set, dataName=testIdx, height=256, width=256)
...
for epoch in range(epoch_num):
    ...
    for idx, (_, real_imgs) in enumerate(TrainSet, 0):
        ...
        try:
            ...
            print("Error in SAmpler.samle_new_exmos:",e)
        # 更新编码器
